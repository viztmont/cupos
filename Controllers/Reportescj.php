<?php 
	class Reportescj extends Controllers{
		...
		public function reportescj(){
			$data['page_tag'] = "Reportes Camiones Jiboa";
			$data['page_title'] = "Reportes Camiones Planta Jiboa";
			$data['page_name'] = "Reportes Camiones Planta Jiboa";
			$data['page_functions_js'] = "functions_reportescj.js";
			$this->views->getView($this,"reportescj",$data);
		}

		public function getReportescj(){
			if($_SESSION['permisosMod']['r']){
				$btnView = '';
				$btnCot = '';
				$arrData = $this->model->selectReportescj();
				for ($i=0; $i < count($arrData); $i++) {
					...
					$btnView = '<button class="btn btn-info btn-sm" onClick="fntViewReportescj('.$arrData[$i]['id_cupo'].')" title="Ver datos"><i class="far fa-eye"></i></button>';
		...
		public function FinalizadoCJ($fi){
			$fechaInicio = ''.$fi;
			$fechaFinal = ''.$_POST['f'];
