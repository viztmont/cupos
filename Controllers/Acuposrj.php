<?php 
    session_start();
	class Acuposrj extends Controllers{
		...
		public function Acuposrj(){
			$data['page_tag'] = "Asignación Cupos Rastras Jiboa";
			$data['page_name'] = "Asignación Cupos Rastra Planta Jiboa";
			$data['page_title'] = "Asignación Cupos Rastra Planta Jiboa";
			$data['page_functions_js'] = "functions_acuposrj.js";
			$this->views->getView($this,"acuposrj",$data);
		}
		...
		public function setAcuporj(){
			...
			if($_POST['listMetodoCargaAR']=='Granel') {
					...
					if ($tiempo=='PM') {
						...
						if ($_POST['listHoraAR']=='04') { $hora = 16; }
					...
					$request_user = $this->model->updateAcuporj($idAcuposrg, $strHora_Asignada, $strCliente, $strMetodo_Carga, $pdfCotizacion, $strRuta, $strEstado, $intTendidos_Tarima, $intCapacidad_Tarima, $intCantidad_Tarima, $strObservacion_Asignacion);
		...
		public function getAcuporj($idcupo){
			$intCupo_Id = intval($idcupo);
			if($intCupo_Id > 0){
				$arrData = $this->model->selectAcuporj($intCupo_Id);
				if(empty($arrData)){
