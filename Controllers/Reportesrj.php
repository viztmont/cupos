<?php 
	class Reportesrj extends Controllers{
		...
		public function reportesrj(){
			$data['page_tag'] = "Reportes Rastras Jiboa";
			$data['page_title'] = "Reportes Rastras Planta Jiboa";
			$data['page_name'] = "Reportes Rastras Planta Jiboa";
			$data['page_functions_js'] = "functions_reportesrj.js";
			$this->views->getView($this,"reportesrj",$data);
		}

		public function getReportesrj(){
			if($_SESSION['permisosMod']['r']){
				$btnView = '';
				$btnCot = '';
				$arrData = $this->model->selectReportesrj();
				for ($i=0; $i < count($arrData); $i++) {
					...
					$btnView = '<button class="btn btn-info btn-sm" onClick="fntViewReportesrj('.$arrData[$i]['id_cupo'].')" title="Ver datos"><i class="far fa-eye"></i></button>';
		...
		/* REPORTE */
		public function FinalizadoRJ($fi){
			$fechaInicio = ''.$fi;
			$fechaFinal = ''.$_POST['f'];
