<?php 
    session_start();
	class Acuposcj extends Controllers{
		...
		public function Acuposcj(){
			$data['page_tag'] = "Asignación Cupos Camiones Jiboa";
			$data['page_name'] = "Asignación Cupos Camiones Planta Jiboa";
			$data['page_title'] = "Asignación Cupos Camiones Planta Jiboa";
			$data['page_functions_js'] = "functions_acuposcj.js";
			$this->views->getView($this,"acuposcj",$data);
		}	
		...
		public function getAcupocj($idcupo){
			$intCupo_Id = intval($idcupo);
			if($intCupo_Id > 0){
				$arrData = $this->model->selectAcupocj($intCupo_Id);
				if(empty($arrData)){
		...
		public function setAcupocj(){
			...
			if ($_POST['listTipoLogisticaAC']=='Transporte propio') {
				...
				if(empty($_POST['listHoraAC']) || empty($_POST['listMinutosAC'])){
					...
					$request_user = $this->model->updateAcupocj($idAcupocg, $strEstado, $strCliente, $strTipo_Logistica, 
