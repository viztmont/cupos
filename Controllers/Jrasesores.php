<?php
    session_start();
	class Jrasesores extends Controllers{
		...
		public function Jrasesores(){
			$data['page_tag'] = "Manejo Rastras Jiboa";
			$data['page_name'] = "Manejo Cupos Rastras Planta Jiboa";
			$data['page_title'] = "Manejo Cupos Rastras Planta Jiboa";
			$data['page_functions_js'] = "functions_jrasesores.js";
			$this->views->getView($this,"jrasesores",$data);
		}	
		...
		/*asignar asesor */
		public function updatec_Asesor_Cupo(){
			if($_POST){
				if($_SESSION['permisosMod']['d']){
					$Cupo_asesor_id = intval($_POST['idCupo_Asesor']);
					$strNombre_asesor = $_POST['listNombresPersona'];
					$request_Update = $this->model->update_Asesor_Cupo($Cupo_asesor_id, $strNombre_asesor);
					if($request_Update){
		...
		/* limpiar asesor */
		public function updatec_Asesor_Cupo_Libre(){
			if($_POST){
				if($_SESSION['permisosMod']['d']){
		...
		/*asignar Tipo Carga*/
		public function tipoCarga_Cupo(){
			if($_POST){
				if($_SESSION['permisosMod']['u']){
					$Cupo_id = intval($_POST['idCupo_Carga']);
					$strTipo_Carga = $_POST['listTipoCarga'];
					$request_Update = $this->model->update_tipoCarga($Cupo_id, $strTipo_Carga);
					if($request_Update){
						$arrResponse = array('status' => true, 'msg' => 'Se ha actualizado el nombre del asesor');
					}else{
						$arrResponse = array('status' => false, 'msg' => 'Error al actualizar.');
		...
		public function set_Nuevo_Cupo(){
			if($_SESSION['permisosMod']['w']){
		    	...
		    	$arrData = $this->model->select_UltimoCupo($intAnio, $intSemana, $intDia, $intBloque);
