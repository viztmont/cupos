<?php 
	class GrasesoresModel extends Mysql{
		public $intIdCupo;
		public $intIdAsesor;
		public $intAnio;
		public $intSemana;
		...
		public function select_Cupos_RG(int $anio, int $semana, int $dia, int $bloque){
			$this->intAnio = $anio;
			$this->intSemana = $semana;
			$this->intDia = $dia;
			$this->intBloque = $bloque;
			$sql = "SELECT COUNT(*) as total FROM `cupos_rastras` WHERE tipo_transporte=1 AND anio=$this->intAnio AND semana=$this->intSemana AND dia=$this->intDia AND bloque=$this->intBloque";
		...
		public function select_SubBloque_Cupos(int $anio, int $semana, int $dia, int $bloque){
			...
			$this->intDia = $dia;
			$this->intBloque = $bloque;
			$sql = "SELECT sub_bloque FROM `cupos_rastras` WHERE tipo_transporte=1 AND anio=$this->intAnio AND semana=$this->intSemana AND dia=$this->intDia AND bloque=$this->intBloque";
			$request = $this->select_all($sql);
			return $request;
		}
		...
		public function select_DatosCupos(int $anio, int $semana, int $dia, int $bloque, int $sub_bloque){
			...
			$sql = "SELECT id_cupo, nombre_asesor, tipo_carga FROM `cupos_rastras`
					WHERE tipo_transporte=1 AND anio=$this->intAnio AND semana=$this->intSemana AND dia=$this->intDia AND bloque=$this->intBloque AND sub_bloque = $this->intSub_bloque";
			$request = $this->select($sql);
			return $request;
		}
		...
		/* actualizar asesor*/
		public function update_Asesor_Cupo_RG(int $idcupo, string $nombre_asesor){
		...
		public function update_tipoCargaRG(int $idcupo, string $tipo_carga){
			$this->intIdCupo = $idcupo;
			$this->strTipo_Carga = $tipo_carga;
			$sql = "UPDATE cupos_rastras SET tipo_carga=? WHERE tipo_transporte=1 AND id_cupo = $this->intIdCupo";
		...
		public function deleteCupo(int $idcupo){
			$this->intIdCupo = $idcupo;
			$sql = "DELETE FROM cupos_rastras WHERE tipo_transporte=1 AND id_cupo = $this->intIdCupo";
		...
		public function select_UltimoCupo_RG(int $anio, int $semana, int $dia, int $bloque){
			$this->intAnio = $anio;
			$this->intSemana = $semana;
			$this->intDia = $dia;
			$this->intBloque = $bloque;
			$sql = "SELECT MAX(sub_bloque) as ultimoCupo FROM cupos_rastras WHERE tipo_transporte=1 AND anio=$this->intAnio AND semana=$this->intSemana AND dia=$this->intDia AND bloque=$this->intBloque ORDER BY sub_bloque DESC";
			$request = $this->select($sql);
