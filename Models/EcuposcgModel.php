<?php 
	...
	class EcuposcgModel extends Mysql{
		public $strDia;
		...
		public function selectEcuposcg(){
			$anio = strftime("%Y");
			$mes = date("m");
			$semana = date("W");
			$dia = date("N");
			$sql = "SELECT * FROM cupos_camiones WHERE tipo_transporte=3 AND anio=$anio AND mes=$mes AND semana=$semana AND dia=$dia";
			$request = $this->select_all($sql);
			return $request;
		}

		public function updateEcupocg(int $idcupo, string $estado, string $motorista, string $camion, string $hora_entrada, string $hora_inicio, string $hora_finalizacion, string $hora_salida, string $tiempo_duracion, string $observacion_ejecucion){
			$this->intCupo_id = $idcupo;
		...
		public function updateEcupocg_TP(int $idcupo, string $estado, string $hora_entrada, string $hora_inicio, string $hora_finalizacion, string $hora_salida, string $tiempo_duracion, string $observacion_ejecucion){
			$this->intCupo_id = $idcupo;
			$this->strEstado = $estado;
			$this->strTiempo_duracion = $tiempo_duracion;
			$this->strHora_entrada = $hora_entrada;
		...
		/* cancelar cupo */
		public function cancelarCupocg(int $idcupo){
			$this->intCupo_id = $idcupo;
		...
		public function selectCamiones() {
			$sql = "SELECT * FROM camiones WHERE planta=1";
			$request = $this->select_all($sql);
			return $request;
		}
		public function selectMotoristas() {
			$sql = "SELECT * FROM motoristas WHERE planta=1 AND nota>=6";
			$request = $this->select_all($sql);
			return $request;
		}		
