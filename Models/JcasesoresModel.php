<?php 
	class JcasesoresModel extends Mysql{
		...
		public function select_Cupos(int $anio, int $semana, int $dia, int $bloque){
			...
			$sql = "SELECT COUNT(*) as total FROM `cupos_camiones` WHERE tipo_transporte=4 AND anio=$this->intAnio AND semana=$this->intSemana AND dia=$this->intDia AND bloque=$this->intBloque";
		...
		public function select_SubBloque_Cupos(int $anio, int $semana, int $dia, int $bloque){
			...
			$this->intDia = $dia;
			$this->intBloque = $bloque;
			$sql = "SELECT sub_bloque FROM `cupos_camiones` WHERE tipo_transporte=4 AND anio=$this->intAnio AND semana=$this->intSemana AND dia=$this->intDia AND bloque=$this->intBloque";
			$request = $this->select_all($sql);
			return $request;
		}
		...
		public function select_DatosCupos(int $anio, int $semana, int $dia, int $bloque, int $sub_bloque){
			...
			$sql = "SELECT id_cupo, nombre_asesor, tipo_logistica FROM `cupos_camiones`
					WHERE tipo_transporte=4 AND anio=$this->intAnio AND semana=$this->intSemana AND dia=$this->intDia AND bloque=$this->intBloque AND sub_bloque = $this->intSub_bloque";
			$request = $this->select($sql);
			return $request;
		}
		...
		public function deleteCupo(int $idcupo){
			$this->intIdCupo = $idcupo;
			$sql = "DELETE FROM cupos_camiones WHERE tipo_transporte=4 AND id_cupo = $this->intIdCupo";
		...
		public function select_UltimoCupo(int $anio, int $semana, int $dia, int $bloque){
			...
			$this->intDia = $dia;
			$this->intBloque = $bloque;
			$sql = "SELECT MAX(sub_bloque) as ultimoCupo FROM cupos_camiones WHERE tipo_transporte=4 AND anio=$this->intAnio AND semana=$this->intSemana AND dia=$this->intDia AND bloque=$this->intBloque ORDER BY sub_bloque DESC";
			$request = $this->select($sql);
