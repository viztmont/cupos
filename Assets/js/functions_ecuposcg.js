...
document.addEventListener('DOMContentLoaded', function(){
	...
	tableEcuposcg = $('#tableEcuposcg').dataTable( {
		"aProcessing":true,
		"aServerSide":true,
        "language": {
        ...
        "ajax":{
            "url": " "+base_url+"/Ecuposcg/getEcuposcg/",
            "dataSrc":""
        },
        "columns":[            
            {"data":"id_cupo"},
    ...
    if(document.querySelector("#formEcuposC")){
        ...
        formAcuposrg.onsubmit = function(e) {
            ...
            let ajaxUrl = base_url+'/Ecuposcg/setEcupocg';
    ...
    if(document.querySelector("#formEcuposCTP")){
        ...
        formAcuposrg.onsubmit = function(e) {
            ...
            divLoading.style.display = "flex";
            let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
            let ajaxUrl = base_url+'/Ecuposcg/setEcupocg_TP';
            let formData = new FormData(formAcuposrg);
            request.open("POST",ajaxUrl,true);
            request.send(formData);
            request.onreadystatechange = function(){
...
function fntEditEcuposcg(idcupo){
...
function fntMotoristas(){
    if(document.querySelector('#listMotoristasEC')){ 
        let ajaxUrl = base_url+'/Ecuposcg/getSelectMotoristas';
        let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
        request.open("GET",ajaxUrl,true);
        request.send();
        request.onreadystatechange = function(){
...
function fntCamiones(){
    if(document.querySelector('#listCamionesEC')){
        let ajaxUrl = base_url+'/Ecuposcg/getSelectCamiones';
        let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
        request.open("GET",ajaxUrl,true);
        request.send();
        request.onreadystatechange = function(){
...
/* ver datos */
function fntViewEcuposcg(idcupo){
    let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    let ajaxUrl = base_url+'/Ecuposcg/getEcupocg/'+idcupo;
    request.open("GET",ajaxUrl,true);
    request.send();
...
function consultaECG() {
    let from_date = $('#txt001CG').val();
    let to_date = $('#txt002CG').val();
    tableEcuposcg = $('#tableEcuposcg').dataTable({
        "aProcessing": true,
        "aServerSide": true,
        "language": {
        ...
        "ajax": {
            "url": " " + base_url + "/Ecuposcg/consultaCG/",
            method: "POST",
...
function fntCanEcuposcg(idcupo){
    swal({
        title: "Cancelar cupo",
        ...
        if (isConfirm) {
            let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
            let ajaxUrl = base_url+'/Ecuposcg/cancelarCupocg';
